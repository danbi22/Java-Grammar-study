...
public class lambdaMain01 {
	...
	public static void main(String[] args) {
		...
		double result = calc.calualate(adder);
		...
		result = calc.calualate(subtractor);
		...
		result = calc.calualate(multiplier);
		System.out.println("result = "+result);
		
		result = calc.calualate(new Calculable() {
			@Override
			public double calculate(double x, double y) {
				return x/y;
			}
		 ...
		 * - 함수형 인터페이스: 추상 메서드를 오직 1개 갖는 인터페이스
		 * 람다 표현식 문법: 
		 * 		(파라미터 선언, ...) -> {메서드 코드;}
