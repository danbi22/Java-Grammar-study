...
public class FileMain02 {
	...
	public static void main(String[] args) {
		// 대용량 파일 복사
		// data/ratings.dat 파일을 읽고 data/ratings-copy.dat 파일 복사
		...
		try {
			...
			long start = System.currentTimeMillis(); // 파일 복사 시작 시간 // 현재 시간을 밀리세컨드 1/1000초 단위로 측정 long 타입으로 리턴해줌
...
//				int read = in.read(); // (1) 파일에서 1byte 읽기
				...
				int read = in.read(buffer); // (2) 파일에서 1바이트 읽기
...
//				out.write(read); // 파일에 1byte 쓰기.
				
				out.write(buffer, 0, read); // 파일에 4KB씩 쓰기.
