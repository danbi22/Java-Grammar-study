...
public class AppMain01 {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() { // EventQueue쓰레드 관리
			@Override
			public void run() {
				try {
	...
	private void initialize() {
		...
		frame.setBounds(1000, 200, 450, 300); // JFrame.setBounds() : JFrame 의 시작 좌표(x, y), 크기(가로, 세로)
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frame.getContentPane().setLayout(new BorderLayout(0, 0));
		
		JLabel lblText = new JLabel("안녕하세요, Swing!");
		lblText.setHorizontalAlignment(SwingConstants.CENTER);
		lblText.setFont(new Font("D2Coding", Font.PLAIN, 22));
		frame.getContentPane().add(lblText, BorderLayout.CENTER);
		
		
		// -> JFrame 우상단의 닫기 버튼(x) 기능을 정의하는 코드 - 프로그램 종료.
	}
