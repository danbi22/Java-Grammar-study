...
//		this.userAge = 0;
	...
	public User(String userId, String userPassword, int userAge) {
		// field를 초기화
		// this - field userId와 parameter userId를 구분하기 위해 사용
		this.userId = userId;
	...
	public User(String userId, String userPassword) {
		this.userId = userId;
		this.userPassword = userPassword;
//		this.userAge = 0 자동으로 초기화해줌
	}
	
	// 생성자 오버로딩(overloading): 파라미터가 다른 생성자를 여러개 정의하는 것.
	// this의 의미(기능):
	// (1) 생성된 객체(인스턴스) - 인스턴스의 필드와 지역 변수를 구분하기 위한 용도
	// (2) overloading된 다른 생성자 호출
	
	// method
	public void printUserInfo() {
		System.out.printf("ID: %s PW: %s Age: %d", userId, userPassword, userAge);
	}
