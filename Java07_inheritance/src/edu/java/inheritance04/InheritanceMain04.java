...
//		((Child) ch2).testChild();  문법적으로 오류가 없다 컴파일시 오류가 발생하지 않느다.
		// 문법 오류는 발생하지 않지만, Child와 AnotherChild는 서로 casting할 수 없는 타입 이기 때문에 실행 중에 에러가 발생
		// 이럴때 instanceof 연잔자: 변수 instanceof 클래스이름
